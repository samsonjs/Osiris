...
### Added
- `CodableRequest<Response>`: type-safe HTTP requests with compile-time response type verification
- Codable body support in HTTPRequest factory methods: `post(body:)`, `put(body:)`, `patch(body:)`
...
- `RequestBuilder` extensions for `CodableRequest` with automatic JSON decoding
- Header manipulation with fluent `adding(header:value:)` API on CodableRequest
